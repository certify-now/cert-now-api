...
/**
 * Dev-only filter that injects a fixed user principal so that API endpoints can
 * be called without a JWT token during local development.
 *
 * <p>Activated only when the {@code dev} Spring profile is active.
 * Has no effect in staging or production.
 *
 * <p>Usage — set the {@code X-Dev-Role} request header:
 * <ul>
 *   <li>{@code X-Dev-Role: CUSTOMER} (default) → dev-customer@certifynow.local</li>
 *   <li>{@code X-Dev-Role: ENGINEER}            → dev-engineer@certifynow.local</li>
 *   <li>{@code X-Dev-Role: ADMIN}               → dev-admin@certifynow.local</li>
 * </ul>
 *
 * <p>If a real {@code Authorization: Bearer} header is present the filter skips
 * injection and lets the normal JWT filter handle it.
 */
@Component
@Profile("dev")
public class DevAuthFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    final String role = (roleHeader != null && !roleHeader.isBlank())
        ? roleHeader.trim().toUpperCase()
        : "CUSTOMER";

    final String userId = switch (role) {
