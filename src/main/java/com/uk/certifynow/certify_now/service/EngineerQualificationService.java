...
public class EngineerQualificationService {
    ...
    public List<EngineerQualificationDTO> findAll() {
        final List<EngineerQualification> engineerQualifications = engineerQualificationRepository.findAll(Sort.by("id"));
        return engineerQualifications.stream()
                .map(engineerQualification -> mapToDTO(engineerQualification, new EngineerQualificationDTO()))
                .toList();
    }

    public EngineerQualificationDTO get(final UUID id) {
        return engineerQualificationRepository.findById(id)
                .map(engineerQualification -> mapToDTO(engineerQualification, new EngineerQualificationDTO()))
    ...
    public void update(final UUID id, final EngineerQualificationDTO engineerQualificationDTO) {
        final EngineerQualification engineerQualification = engineerQualificationRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(engineerQualificationDTO, engineerQualification);
    ...
    private EngineerQualificationDTO mapToDTO(final EngineerQualification engineerQualification,
        ...
        engineerQualificationDTO.setEngineerProfile(engineerQualification.getEngineerProfile() == null ? null : engineerQualification.getEngineerProfile().getId());
    ...
    private EngineerQualification mapToEntity(
        ...
        final EngineerProfile engineerProfile = engineerQualificationDTO.getEngineerProfile() == null ? null : engineerProfileRepository.findById(engineerQualificationDTO.getEngineerProfile())
    ...
    public void on(final BeforeDeleteEngineerProfile event) {
        ...
        final EngineerQualification engineerProfileEngineerQualification = engineerQualificationRepository.findFirstByEngineerProfileId(event.getId());
        if (engineerProfileEngineerQualification != null) {
            referencedException.setKey("engineerProfile.engineerQualification.engineerProfile.referenced");
