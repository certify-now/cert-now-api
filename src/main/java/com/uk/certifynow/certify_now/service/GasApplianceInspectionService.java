...
public class GasApplianceInspectionService {
  ...
  public List<GasApplianceInspectionDTO> findAll() {
    final List<GasApplianceInspection> gasApplianceInspections =
        gasApplianceInspectionRepository.findAll(Sort.by("id"));
    return gasApplianceInspections.stream()
        .map(
            gasApplianceInspection ->
                mapToDTO(gasApplianceInspection, new GasApplianceInspectionDTO()))
        .toList();
  }

  public GasApplianceInspectionDTO get(final UUID id) {
    return gasApplianceInspectionRepository
        .findById(id)
        .map(
            gasApplianceInspection ->
                mapToDTO(gasApplianceInspection, new GasApplianceInspectionDTO()))
  ...
  public void update(final UUID id, final GasApplianceInspectionDTO gasApplianceInspectionDTO) {
    final GasApplianceInspection gasApplianceInspection =
        gasApplianceInspectionRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(gasApplianceInspectionDTO, gasApplianceInspection);
  ...
  private GasApplianceInspectionDTO mapToDTO(
      final GasApplianceInspection gasApplianceInspection,
    ...
    gasApplianceInspectionDTO.setOperatingPressureMbar(
        gasApplianceInspection.getOperatingPressureMbar());
    gasApplianceInspectionDTO.setRatio(gasApplianceInspection.getRatio());
    gasApplianceInspectionDTO.setSafetyDeviceCorrect(
        gasApplianceInspection.getSafetyDeviceCorrect());
    ...
    gasApplianceInspectionDTO.setGasInspection(
        gasApplianceInspection.getGasInspection() == null
            ? null
            : gasApplianceInspection.getGasInspection().getId());
  ...
  private GasApplianceInspection mapToEntity(
    ...
    gasApplianceInspection.setOperatingPressureMbar(
        gasApplianceInspectionDTO.getOperatingPressureMbar());
    gasApplianceInspection.setRatio(gasApplianceInspectionDTO.getRatio());
    gasApplianceInspection.setSafetyDeviceCorrect(
        gasApplianceInspectionDTO.getSafetyDeviceCorrect());
    ...
    final GasSafetyInspection gasInspection =
        gasApplianceInspectionDTO.getGasInspection() == null
            ? null
            : gasSafetyInspectionRepository
                .findById(gasApplianceInspectionDTO.getGasInspection())
  ...
  public void on(final BeforeDeleteGasSafetyInspection event) {
    ...
    final GasApplianceInspection gasInspectionGasApplianceInspection =
        gasApplianceInspectionRepository.findFirstByGasInspectionId(event.getId());
    if (gasInspectionGasApplianceInspection != null) {
      referencedException.setKey(
          "gasSafetyInspection.gasApplianceInspection.gasInspection.referenced");
