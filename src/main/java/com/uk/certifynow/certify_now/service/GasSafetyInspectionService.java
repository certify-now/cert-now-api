...
public class GasSafetyInspectionService {
  ...
  public List<GasSafetyInspectionDTO> findAll() {
    final List<GasSafetyInspection> gasSafetyInspections =
        gasSafetyInspectionRepository.findAll(Sort.by("id"));
  ...
  public GasSafetyInspectionDTO get(final UUID id) {
    return gasSafetyInspectionRepository
        .findById(id)
        .map(gasSafetyInspection -> mapToDTO(gasSafetyInspection, new GasSafetyInspectionDTO()))
  ...
  public void update(final UUID id, final GasSafetyInspectionDTO gasSafetyInspectionDTO) {
    final GasSafetyInspection gasSafetyInspection =
        gasSafetyInspectionRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(gasSafetyInspectionDTO, gasSafetyInspection);
  ...
  private GasSafetyInspectionDTO mapToDTO(
      final GasSafetyInspection gasSafetyInspection,
    ...
    gasSafetyInspectionDTO.setJob(
        gasSafetyInspection.getJob() == null ? null : gasSafetyInspection.getJob().getId());
    return gasSafetyInspectionDTO;
  }

  private GasSafetyInspection mapToEntity(
      final GasSafetyInspectionDTO gasSafetyInspectionDTO,
      final GasSafetyInspection gasSafetyInspection) {
    ...
    final Job job =
        gasSafetyInspectionDTO.getJob() == null
            ? null
            : jobRepository
                .findById(gasSafetyInspectionDTO.getJob())
  ...
  public void on(final BeforeDeleteJob event) {
    ...
    final GasSafetyInspection jobGasSafetyInspection =
        gasSafetyInspectionRepository.findFirstByJobId(event.getId());
