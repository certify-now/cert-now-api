...
public class JobMatchLogService {
    ...
    public JobMatchLogService(final JobMatchLogRepository jobMatchLogRepository,
    ...
    public JobMatchLogDTO get(final UUID id) {
        return jobMatchLogRepository.findById(id)
                .map(jobMatchLog -> mapToDTO(jobMatchLog, new JobMatchLogDTO()))
    ...
    public void update(final UUID id, final JobMatchLogDTO jobMatchLogDTO) {
        final JobMatchLog jobMatchLog = jobMatchLogRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(jobMatchLogDTO, jobMatchLog);
    ...
    private JobMatchLogDTO mapToDTO(final JobMatchLog jobMatchLog,
            final JobMatchLogDTO jobMatchLogDTO) {
        ...
        jobMatchLogDTO.setEngineer(jobMatchLog.getEngineer() == null ? null : jobMatchLog.getEngineer().getId());
        jobMatchLogDTO.setJob(jobMatchLog.getJob() == null ? null : jobMatchLog.getJob().getId());
        return jobMatchLogDTO;
    }

    private JobMatchLog mapToEntity(final JobMatchLogDTO jobMatchLogDTO,
        ...
        final User engineer = jobMatchLogDTO.getEngineer() == null ? null : userRepository.findById(jobMatchLogDTO.getEngineer())
                .orElseThrow(() -> new NotFoundException("engineer not found"));
        jobMatchLog.setEngineer(engineer);
        final Job job = jobMatchLogDTO.getJob() == null ? null : jobRepository.findById(jobMatchLogDTO.getJob())
    ...
    public void on(final BeforeDeleteUser event) {
        ...
        final JobMatchLog engineerJobMatchLog = jobMatchLogRepository.findFirstByEngineerId(event.getId());
