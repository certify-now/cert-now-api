...
public class PayoutService {
    ...
    public void update(final UUID id, final PayoutDTO payoutDTO) {
        final Payout payout = payoutRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(payoutDTO, payout);
    ...
    private Payout mapToEntity(final PayoutDTO payoutDTO, final Payout payout) {
        ...
        final User engineer = payoutDTO.getEngineer() == null ? null : userRepository.findById(payoutDTO.getEngineer())
                .orElseThrow(() -> new NotFoundException("engineer not found"));
        payout.setEngineer(engineer);
        final Job job = payoutDTO.getJob() == null ? null : jobRepository.findById(payoutDTO.getJob())
                .orElseThrow(() -> new NotFoundException("job not found"));
        payout.setJob(job);
        final Payment payment = payoutDTO.getPayment() == null ? null : paymentRepository.findById(payoutDTO.getPayment())
