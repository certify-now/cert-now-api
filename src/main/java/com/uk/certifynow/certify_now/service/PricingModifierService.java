...
public class PricingModifierService {
  ...
  public PricingModifierDTO get(final UUID id) {
    return pricingModifierRepository
        .findById(id)
        .map(pricingModifier -> mapToDTO(pricingModifier, new PricingModifierDTO()))
  ...
  public void update(final UUID id, final PricingModifierDTO pricingModifierDTO) {
    final PricingModifier pricingModifier =
        pricingModifierRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(pricingModifierDTO, pricingModifier);
  ...
  private PricingModifierDTO mapToDTO(
      final PricingModifier pricingModifier, final PricingModifierDTO pricingModifierDTO) {
    ...
    pricingModifierDTO.setPricingRule(
        pricingModifier.getPricingRule() == null ? null : pricingModifier.getPricingRule().getId());
    return pricingModifierDTO;
  }

  private PricingModifier mapToEntity(
      final PricingModifierDTO pricingModifierDTO, final PricingModifier pricingModifier) {
    ...
    final PricingRule pricingRule =
        pricingModifierDTO.getPricingRule() == null
            ? null
            : pricingRuleRepository
                .findById(pricingModifierDTO.getPricingRule())
  ...
  public void on(final BeforeDeletePricingRule event) {
    ...
    final PricingModifier pricingRulePricingModifier =
        pricingModifierRepository.findFirstByPricingRuleId(event.getId());
