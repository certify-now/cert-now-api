...
public class RenewalReminderService {
    ...
    public RenewalReminderService(final RenewalReminderRepository renewalReminderRepository,
            final CertificateRepository certificateRepository, final UserRepository userRepository,
    ...
    public void update(final UUID id, final RenewalReminderDTO renewalReminderDTO) {
        final RenewalReminder renewalReminder = renewalReminderRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(renewalReminderDTO, renewalReminder);
    ...
    private RenewalReminderDTO mapToDTO(final RenewalReminder renewalReminder,
            final RenewalReminderDTO renewalReminderDTO) {
        ...
        renewalReminderDTO.setCertificate(renewalReminder.getCertificate() == null ? null : renewalReminder.getCertificate().getId());
        renewalReminderDTO.setCustomer(renewalReminder.getCustomer() == null ? null : renewalReminder.getCustomer().getId());
        renewalReminderDTO.setNotification(renewalReminder.getNotification() == null ? null : renewalReminder.getNotification().getId());
        renewalReminderDTO.setProperty(renewalReminder.getProperty() == null ? null : renewalReminder.getProperty().getId());
        return renewalReminderDTO;
    }

    private RenewalReminder mapToEntity(final RenewalReminderDTO renewalReminderDTO,
        ...
        final Certificate certificate = renewalReminderDTO.getCertificate() == null ? null : certificateRepository.findById(renewalReminderDTO.getCertificate())
                .orElseThrow(() -> new NotFoundException("certificate not found"));
        renewalReminder.setCertificate(certificate);
        final User customer = renewalReminderDTO.getCustomer() == null ? null : userRepository.findById(renewalReminderDTO.getCustomer())
                .orElseThrow(() -> new NotFoundException("customer not found"));
        renewalReminder.setCustomer(customer);
        final Notification notification = renewalReminderDTO.getNotification() == null ? null : notificationRepository.findById(renewalReminderDTO.getNotification())
                .orElseThrow(() -> new NotFoundException("notification not found"));
        renewalReminder.setNotification(notification);
        final Property property = renewalReminderDTO.getProperty() == null ? null : propertyRepository.findById(renewalReminderDTO.getProperty())
    ...
    public void on(final BeforeDeleteCertificate event) {
        ...
        final RenewalReminder certificateRenewalReminder = renewalReminderRepository.findFirstByCertificateId(event.getId());
    ...
    public void on(final BeforeDeleteUser event) {
        final ReferencedException referencedException = new ReferencedException();
        final RenewalReminder customerRenewalReminder = renewalReminderRepository.findFirstByCustomerId(event.getId());
    ...
    public void on(final BeforeDeleteNotification event) {
        final ReferencedException referencedException = new ReferencedException();
        final RenewalReminder notificationRenewalReminder = renewalReminderRepository.findFirstByNotificationId(event.getId());
    ...
    public void on(final BeforeDeleteProperty event) {
        final ReferencedException referencedException = new ReferencedException();
        final RenewalReminder propertyRenewalReminder = renewalReminderRepository.findFirstByPropertyId(event.getId());
