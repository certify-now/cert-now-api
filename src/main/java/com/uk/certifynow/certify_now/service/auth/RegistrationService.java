 ...
 *
 * <p>
 * Responsibilities: - Validate email/phone uniqueness - Create User aggregate
 * via UserFactory -
 * Create profile via ProfileFactory - Create user consents - Publish
 * UserRegisteredEvent
 *
 * <p>
 * This service is focused solely on registration logic and delegates entity
 * creation to domain
 * factories, ensuring the application layer doesn't know about business
 * defaults.
 */
@Service
public class RegistrationService {
  ...
  public User registerUser(final RegisterRequest request, final String ipAddress) {
    ...
    final User user = userFactory.createEmailUser(
        request.email(),
        request.password(),
        request.fullName(),
        request.phone(),
   ...
   *
   * <p>
   * Uses ProfileFactory to ensure business defaults are set correctly.
   */
