package com.uk.certifynow.certify_now.service.gdpr;
...
public class DataRequestService {
  ...
  public DataRequestService(
      final DataRequestRepository dataRequestRepository, final UserRepository userRepository) {
  ...
  public void update(final UUID id, final DataRequestDTO dataRequestDTO) {
    final DataRequest dataRequest =
        dataRequestRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(dataRequestDTO, dataRequest);
  ...
  private DataRequestDTO mapToDTO(
      final DataRequest dataRequest, final DataRequestDTO dataRequestDTO) {
  ...
  private DataRequest mapToEntity(
      final DataRequestDTO dataRequestDTO, final DataRequest dataRequest) {
    ...
    final User user =
        dataRequestDTO.getUser() == null
            ? null
            : userRepository
                .findById(dataRequestDTO.getUser())
