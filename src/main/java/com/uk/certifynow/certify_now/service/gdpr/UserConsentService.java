...
public class UserConsentService {
  ...
  public UserConsentService(
      final UserConsentRepository userConsentRepository, final UserRepository userRepository) {
  ...
  public void update(final UUID id, final UserConsentDTO userConsentDTO) {
    final UserConsent userConsent =
        userConsentRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(userConsentDTO, userConsent);
  ...
  private UserConsentDTO mapToDTO(
      final UserConsent userConsent, final UserConsentDTO userConsentDTO) {
  ...
  private UserConsent mapToEntity(
      final UserConsentDTO userConsentDTO, final UserConsent userConsent) {
    ...
    final User user =
        userConsentDTO.getUser() == null
            ? null
            : userRepository
                .findById(userConsentDTO.getUser())
