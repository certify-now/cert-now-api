...
public class JobStatusHistoryService {
  ...
  public JobStatusHistoryService(
      final JobStatusHistoryRepository jobStatusHistoryRepository,
  ...
  public List<JobStatusHistoryDTO> findAll() {
    final List<JobStatusHistory> jobStatusHistories =
        jobStatusHistoryRepository.findAll(Sort.by("id"));
  ...
  public void update(final UUID id, final JobStatusHistoryDTO jobStatusHistoryDTO) {
    final JobStatusHistory jobStatusHistory =
        jobStatusHistoryRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(jobStatusHistoryDTO, jobStatusHistory);
  ...
  private JobStatusHistoryDTO mapToDTO(
      final JobStatusHistory jobStatusHistory, final JobStatusHistoryDTO jobStatusHistoryDTO) {
    ...
    jobStatusHistoryDTO.setJob(
        jobStatusHistory.getJob() == null ? null : jobStatusHistory.getJob().getId());
    return jobStatusHistoryDTO;
  }

  private JobStatusHistory mapToEntity(
      final JobStatusHistoryDTO jobStatusHistoryDTO, final JobStatusHistory jobStatusHistory) {
    ...
    final Job job =
        jobStatusHistoryDTO.getJob() == null
            ? null
            : jobRepository
                .findById(jobStatusHistoryDTO.getJob())
  ...
  public void on(final BeforeDeleteJob event) {
    ...
    final JobStatusHistory jobJobStatusHistory =
        jobStatusHistoryRepository.findFirstByJobId(event.getId());
