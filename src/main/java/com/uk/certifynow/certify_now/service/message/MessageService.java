package com.uk.certifynow.certify_now.service.message;
...
public class MessageService {
  ...
  public List<MessageDTO> findAll() {
    ...
    return messages.stream().map(message -> mapToDTO(message, new MessageDTO())).toList();
  }
  ...
  public void update(final UUID id, final MessageDTO messageDTO) {
    final Message message = messageRepository.findById(id).orElseThrow(NotFoundException::new);
    mapToEntity(messageDTO, message);
  ...
  private Message mapToEntity(final MessageDTO messageDTO, final Message message) {
    ...
    final Job job =
        messageDTO.getJob() == null
            ? null
            : jobRepository
                .findById(messageDTO.getJob())
