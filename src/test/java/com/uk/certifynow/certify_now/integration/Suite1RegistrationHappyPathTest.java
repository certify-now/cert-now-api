...
// TestDataFactory.RegisterPayload.customer(email);
//
...
//        jdbcTemplate.queryForObject(
//            "SELECT COUNT(*) FROM customer_profile cp JOIN \"user\" u ON cp.user_id = u.id WHERE
// LOWER(u.email) = LOWER(?)",
//            Integer.class,
//            email);
...
//        jdbcTemplate.queryForObject(
//            "SELECT COUNT(*) FROM user_consent uc JOIN \"user\" u ON uc.user_id = u.id WHERE
// LOWER(u.email) = LOWER(?)",
//            Integer.class,
//            email);
...
//    final String email = TestDataFactory.uniqueEmail();
//    final TestDataFactory.RegisterPayload payload =
// TestDataFactory.RegisterPayload.engineer(email);
//
//    final ValidatableResponse response = register(payload);
//
//    response
...
//    final Integer profileCount =
//        jdbcTemplate.queryForObject(
//            "SELECT COUNT(*) FROM engineer_profile ep JOIN \"user\" u ON ep.user_id = u.id WHERE
// LOWER(u.email) = LOWER(?)",
//            Integer.class,
//            email);
//    assertThat(profileCount).isEqualTo(1);
